...
export default {
  ...
  props: ['card'],
  methods: {
    flipCard(card: ICard, event: Event) {
      const target = event.target as HTMLElement;
      if (target && target.parentNode && target.parentNode.nodeName !== 'svg') {
...
<template>
  <div class="card-box" :class="card.color">
    <transition name="flip">
      <article @click="flipCard(card, $event)" :key="card.flipped" class="single-card">
...
.card-box {
  ...
  &.-orange .single-card {
    ...
    box-shadow: 0 0 12px 1px rgba($card-orange, 0.3);
    &:hover {
      box-shadow: 0 0 14px 2px rgba($card-orange, 0.7);
    }
  }
  &.-red .single-card {
    background-color: $card-red;
    box-shadow: 0 0 8px 1px rgba($card-red, 0.3);
    &:hover {
      box-shadow: 0 0 14px 2px rgba($card-red, 0.7);
    }
  }
  &.-purple .single-card {
    background-color: $card-purple;
    box-shadow: 0 0 12px 1px rgba($card-purple, 0.3);
    &:hover {
      box-shadow: 0 0 14px 2px rgba($card-purple, 0.7);
    }
  }
  &.-blue .single-card {
    background-color: $card-blue;
    box-shadow: 0 0 8px 1px rgba($card-blue, 0.3);
    &:hover {
      box-shadow: 0 0 14px 2px rgba($card-blue, 0.7);
    }
  }
  &.-green .single-card {
    background-color: $card-green;
    box-shadow: 0 0 8px 1px rgba($card-green, 0.3);
    &:hover {
      box-shadow: 0 0 14px 2px rgba($card-green, 0.7);
    }
  }
}
